#!/bin/bash
#From 
# http://www.linuxquestions.org/questions/programming-9/linux-script-limiting-zip-archive-size-911631/#post4514989
#
# This version does not uses compression, just archives the files to a zip file.
...
find "$@" -type f -print0 | (
    ...
    while read -d "" FILE ; do
        ...
        INDEX=$[ INDEX + 1 ]
        zip -0 "$BASENAME-$INDEX.zip" "${FILES[@]}" || exit $?
