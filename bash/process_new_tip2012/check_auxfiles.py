...
def draw_png(item_flac, item_wav, sampling_rate, max_freq_draw, spectrogram_palette):
	...
	if status != 0:
		...
		print output
		sys.exit(1)
	text1 = "convert -fill " + letter_color + " -draw \"text 5,15 '" + max_freq_draw_t + "'\" -draw \"text 5,155 '" + half_max_freq_draw_t + "'\" " + item_flac[:-5] + "_s1.png -quality 10 " + item_flac[:-5] + "_s.png"
	...
	text1 = "convert " + item_flac[:-5] + "_w1.png -quality 10 " + item_flac[:-5] + "_w.png"
	status, output = commands.getstatusoutput(text1)
	if status != 0:
		print " "
		print "There was a problem processing " + item_flac + "!"
	...
	if status != 0:
		...
		print output
		sys.exit(1)
	text1 = "convert -fill " + letter_color + " -draw \"text 5,15 '" + max_freq_draw_t + "'\" " + item_flac[:-5] + "-small_s1.png -quality 10 " + item_flac[:-5] + "-small_s.png"
	status, output = commands.getstatusoutput(text1)
	if status != 0:
		print " "
		print "There was a problem processing " + item_flac + "!"
	...
	text1 = "convert " + item_flac[:-5] + "-small_w1.png -quality 10 " + item_flac[:-5] + "-small_w.png"
	status, output = commands.getstatusoutput(text1)
	if status != 0:
		print " "
		print "There was a problem processing " + item_flac + "!"
	...
	if status != 0:
		...
		print output
		sys.exit(1)
	text1 = "convert -fill " + letter_color + " -draw \"text 5,15 '" + max_freq_draw_t + "'\" -draw \"text 5,235 '" + half_max_freq_draw_t + "'\" " + item_flac[:-5] + "-large_s1.png -quality 10 " + item_flac[:-5] + "-large_s.png"
	status, output = commands.getstatusoutput(text1)
	if status != 0:
		print " "
		print "There was a problem processing " + item_flac + "!"
	...
	text1 = "convert  " + item_flac[:-5] + "-large_w1.png -quality 10 " + item_flac[:-5] + "-large_w.png"
	status, output = commands.getstatusoutput(text1)
	if status != 0:
		print " "
		print "There was a problem processing " + item_flac + "!"
...
#Draw Waveform and Spectrogram to png files
def draw_png_stereo(item_flac, item_wav, sampling_rate, max_freq_draw, spectrogram_palette):
	...
	text1 = "convert -fill " + letter_color + " -draw \"text 5,15 '" + max_freq_draw_t + "'\" -draw \"text 5,165 '" + max_freq_draw_t + "'\" -draw \"text 590,15 'L'\" -draw \"text 590,165 'R'\" sall.png -quality 10 " + item_flac[:-5] + "_s.png"
	status, output = commands.getstatusoutput(text1)
	if status != 0:
		print " "
		print "There was a problem processing " + item_flac + "!"
		print output
		sys.exit(1)	
	status, output = commands.getstatusoutput("convert -fill " + letter_color + " -draw \"text 590,15 'L'\" -draw \"text 590,165 'R'\" wall.png -quality 10 " + item_flac[:-5] + '_w.png')
	if status != 0:
		print " "
		print "There was a problem processing " + item_flac + "!"
		print output
	...
	if status != 0:
		...
		print output
		sys.exit(1)
	text1 = "convert -fill " + letter_color + " -draw \"text 5,15 '" + max_freq_draw_t + "'\" -draw \"text 5,85 '" + max_freq_draw_t + "'\" -draw \"text 290,15 'L'\" -draw \"text 290,85 'R'\" sall.png -quality 10 " + item_flac[:-5] + "-small_s.png"
	status, output = commands.getstatusoutput(text1)
	if status != 0:
		print " "
		print "There was a problem processing " + item_flac + "!"
		print output
		sys.exit(1)	
	status, output = commands.getstatusoutput("convert -fill " + letter_color + " -draw \"text 290,15 'L'\" -draw \"text 290,85 'R'\" wall.png -quality 10 " + item_flac[:-5] + '-small_w.png')
	if status != 0:
		print " "
		print "There was a problem processing " + item_flac + "!"
		print output
	...
	if status != 0:
		...
		print output
		sys.exit(1)	
	text1 = "convert -fill " + letter_color + " -draw \"text 5,15 '" + max_freq_draw_t + "'\" -draw \"text 5,245 '" + max_freq_draw_t + "'\" -draw \"text 905,15 'L'\" -draw \"text 905,245 'R'\" sall.png -quality 10 " + item_flac[:-5] + "-large_s.png"
	status, output = commands.getstatusoutput(text1)
	if status != 0:
		print " "
		print "There was a problem processing " + item_flac + "!"
		print output
		sys.exit(1)	
	status, output = commands.getstatusoutput("convert -fill " + letter_color + " -draw \"text 905,15 'L'\" -draw \"text 905,245 'R'\" wall.png -quality 10 " + item_flac[:-5] + '-large_w.png')
	if status != 0:
		print " "
		print "There was a problem processing " + item_flac + "!"
		print output
...
def makemp3(item_wav,item_flac):
	#Convert wav to mp3
	mp3file = item_flac[:-5] + '.autopreview.mp3'
...
def cleanup(server_dir,colID,item_flac,newpng,newmp3):
	pathToUse = server_dir
	pathToPNG = pathToUse + 'sounds/images/' + colID + '/'
	pathToMP3 = pathToUse + 'sounds/previewsounds/' + colID + '/'
	pathToSound = pathToUse + 'sounds/sounds/' + colID + '/'
...
def insertmp3(SoundID,mp3filename):
	...
	try:
		...
		sys.exit (1)
	cursor = con.cursor()
	query = "UPDATE Sounds SET MP3Filename=" + \
...
def insertpng(SoundID, soundname, spectrogram_palette, max_freq):
	...
	small_w=soundname_prefix + '-small_w.png'
	small_s=soundname_prefix + '-small_s.png'
	med_w=soundname_prefix + '_w.png'
	med_s=soundname_prefix + '_s.png'
	large_w=soundname_prefix + '-large_w.png'
	large_s=soundname_prefix + '-large_s.png'
...
def getallsounds():
	...
	try:
		...
		sys.exit (1)
	cursor = con.cursor()
	query = "SELECT SoundID, ColID, OriginalFilename, SoundFormat FROM Sounds WHERE SoundFormat='flac' ORDER BY RAND()"
	cursor.execute (query)
...
def checkpngfiles(SoundID, ColID):
	SoundID = str(SoundID)
	ColID = str(ColID)
	check_val=0
	#Open MySQL
	...
	if cursor.rowcount == 1:
		...
		if fileExists(server_dir + 'sounds/images/' + ColID + '/' + this_image):
...
def checkmp3file(SoundID,ColID):
	SoundID = str(SoundID)
	ColID = str(ColID)
	check_val=0
	#Open MySQL
	...
	cursor = con.cursor()
	#
	cursor.execute("""SELECT MP3Filename FROM Sounds WHERE SoundID=%s""", (SoundID,))
	this_file = cursor.fetchone()
	if type(this_file[0]) != types.NoneType:
		this_file = this_file[0]
		#
		if fileExists(server_dir + 'sounds/previewsounds/' + ColID + '/' + this_file):
			check_val=check_val+1
	#
	cursor.close ()
	con.close ()
...
try:
	for row in results:
		...
		c2=int(checkpngfiles(SoundID, ColID))
		c3=int(checkmp3file(SoundID, ColID))
		c4=c1 + c2 + c3

		filez = check_filesize(SoundID)
		if filez == 0:
			this_file_size = int(os.path.getsize(server_dir + 'sounds/sounds/' + ColID + '/' + item_flac))
			insert_filesize(SoundID,this_file_size)

		if fileExists(server_dir + 'sounds/sounds/' + ColID + '/' + item_flac)!=1:
			print " Original flac file could not be found!"
			print server_dir + 'sounds/sounds/' + ColID + '/' + item_flac
			sys.exit (1)
					
		if c4!=13:
			with Ticker("\n Extracting file from flac " + ColID + "/" + item_flac + " (ID: " + SoundID + ")"):
				item_wav = extractflac(server_dir + 'sounds/sounds/' + ColID + '/' + item_flac, item_flac)
				#Check if stereo
			...
			with Ticker("\n Generating images..."):
				if no_channels == 2:
					delmysql(SoundID)
					draw_png_stereo(item_flac, item_wav, sampling_rate, max_freq_draw, spectrogram_palette)
					insertpng(SoundID, item_flac, spectrogram_palette,max_freq_draw)
				if no_channels == 1:
					delmysql(SoundID)
					draw_png(item_flac, item_wav, sampling_rate, max_freq_draw, spectrogram_palette)
					insertpng(SoundID, item_flac, spectrogram_palette,max_freq_draw)

			mp3newfile=0
			if c3!=1:
				with Ticker("\n Generating mp3 file..." + ColID + "/" + item_flac + " (ID: " + SoundID + ")"):
					mp3filename = makemp3(item_wav,item_wav)
					insertmp3(SoundID,mp3filename)
					mp3newfile=1
										
			cleanup(server_dir,ColID,item_flac,1,mp3newfile)
			...
			if c3!=1:
				with Ticker("\n Extracting file from flac "  + ColID + "/" + item_flac + " (ID: " + SoundID + ")"):
					item_wav = extractflac(server_dir + 'sounds/sounds/' + ColID + '/' + item_flac, item_flac)
				with Ticker("\n Generating mp3 file..." + ColID + "/" + item_flac + " (ID: " + SoundID + ")"):
					mp3filename = makemp3(item_wav,item_flac)
					insertmp3(SoundID,mp3filename)	
					mp3newfile=1

			cleanup(server_dir,ColID,item_flac,pngnewfile,mp3newfile)

		c5 = int(checkmd5(SoundID))
		if c5!=0:
			with Ticker("\n Storing MD5 hash of file " + ColID + "/" + item_flac + " ID: " + SoundID):
				filemd5 = getmd5(server_dir + 'sounds/sounds/' + ColID + '/' + item_flac)
