...
if len(sys.argv) < 2:
	...
	print "\n Optional values are: <Collection ID> <Site ID>\n"
	print "\n Exiting program.\n"
	sys.exit(1)
...
def insert(ColID, soundname, mysql_name, soundformat, no_channels, samplingrate, bitres, soundlength, sounddate, soundtime, file_md5, filesize, siteid, DirID):
	#Open MySQL
	try:
		...
		sys.exit (1)
	cursor = con.cursor()
	query = "INSERT INTO Sounds (ColID, SoundName, OriginalFilename, SoundFormat, Channels, SamplingRate, BitRate, Duration, Date, Time, MD5_hash, FileSize, SiteID, DirID) \
         VALUES (" + \
	`ColID` + ', ' + `soundname` + ', ' + `mysql_name` + ', ' + `soundformat` + ', ' + `no_channels` + ', ' + `samplingrate` + ', ' + `bitres` + ', ' + `soundlength` + ', ' + `sounddate` + ', ' + `soundtime` + ', ' + `file_md5` +  ', ' + `filesize` + ', ' + `siteid` + ', ' + `DirID` + ')'
	#print "Query: " + query + "\n"
...
def tomysql(ColID, item_wav, item_flac, file_md5, siteid, DirID):
	...
	SoundID=insert(ColID, soundname, item_flac, 'flac', wave_vars['no_channels'], wave_vars['samp_rate'], wave_vars['bits'], wave_vars['max_time'], SampDate, SampTime, file_md5, str(filesize), siteid, DirID)
	...
	for item_flac in ls:
		if item_flac[-5:] == ".flac" or item_flac[-5:] == ".FLAC":
			...
			with Ticker("\n Checking the file and inserting the data to MySQL..."):
				SoundID=tomysql(ColID, item_wav, item_flac, file_md5, str(siteid), DirID)
			print " "
			with Ticker("\n Cleaning up..."):
